...
int main(int argc, char **argv) {
  ...
  int i=0, nIter=2500, j=0, jMax=0;
  FILE *fp;
  char *fn=malloc(sizeof(*fn)*(strlen("results/w5/spatTempColNoise_s0000_t0000Fourier.dat")+1));
  ...
  for(i=1; i<=nIter; i++) {
    ...
    grf_initColouredNoise(zeta, w, tau, lambda,
			  1, N, 0.05);
    sprintf(fn, "results/w5/spatTempColNoise_s%04i_t%04iFourier.dat", i,j);
    fp=fopen(fn, "w");
    grf_printCompMatrix(fp, (const double **)zeta, N);
    fclose(fp);
  
    sprintf(fn, "results/w5/spatTempColNoise_s%04i_t%04i.dat", i,j);
